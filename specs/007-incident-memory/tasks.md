...
- [X] T008 [P] Extend `backend/infra/config.py` — add `MemorySettings` (`enabled=True`, `backend: Literal["graphiti","pgvector"]="graphiti"`, `neo4j_uri="bolt://neo4j:7687"`, `neo4j_vault_path="secret/memory"`, `retrieval_k=5` `gt=0`, `retrieval_timeout_s=5.0` `gt=0`, `embedding_model="text-embedding-004"`); add `"memory"` to `_KNOWN_ARGUS_SECTIONS`; add `memory: MemorySettings` to `Settings`; add a `model_validator` appending `neo4j_vault_path` to `vault.required_paths` (fail-boot if unseeded — mirror `_ensure_llm_vault_path_required`). (data-model §config; research MD5)
...
> Not part of the default path. Flip `ARGUS__MEMORY__BACKEND=pgvector` and build the decided fallback

...
**Why this priority**: It is the "gets smarter" enhancement that turns a one-shot external lookup into accumulated, temporal institutional memory — the behaviour that distinguishes Sentinel's knowledge layer from a stateless intel proxy. It ranks below the corpus because the system is fully demonstrable with the seeded corpus alone, and the brief marks it optional for v1.

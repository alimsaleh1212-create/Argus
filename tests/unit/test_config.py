...
class TestSettingsValidation:
    def test_valid_settings_loads(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.delenv("SENTINEL__APP__ENV", raising=False)
    ...
    def test_unknown_extra_key_raises(self, monkeypatch: pytest.MonkeyPatch) -> None:
        """Unknown SENTINEL__ env key must cause ValueError (FR-002)."""
        monkeypatch.setenv("SENTINEL__UNKNOWN_KEY", "boom")
        from backend.infra.config import load_settings

        with pytest.raises(ValueError, match="Unknown SENTINEL__"):
            load_settings()

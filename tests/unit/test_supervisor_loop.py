...
async def test_full_depth_ambiguous_incident_reaches_resolved():
    ...
    async def response(inc):
        ...
        return StageResult(
            stage=StageName.RESPONSE, outcome=StageOutcome.RESOLVED, disposition="auto_remediated"
        )
...
async def test_incident_never_left_in_flight():
    ...
    async def enrichment(inc):
        return StageResult(stage=StageName.ENRICHMENT, outcome=StageOutcome.ESCALATE)
...
async def test_triage_escalate_lands_in_escalated():
    async def triage(inc):
        return StageResult(stage=StageName.TRIAGE, outcome=StageOutcome.ESCALATE)
